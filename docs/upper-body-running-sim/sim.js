...
function clampInput(el,value){const min=Number(el.min),max=Number(el.max),step=Number(el.step)||1;let v=Math.max(min,Math.min(max,value));const decimals=(String(step).split('.')[1]||'').length;v=Number(v.toFixed(decimals));el.value=v;el.dispatchEvent(new Event('input',{bubbles:true}));}
function refreshLabels(){ids.forEach(id=>document.getElementById(id+'Val').textContent=fmt[id](ui[id].value));}
ids.forEach(id=>ui[id].addEventListener('input',()=>document.getElementById(id+'Val').textContent=fmt[id](ui[id].value)));
document.querySelectorAll('.step-btn').forEach(btn=>btn.addEventListener('click',()=>{const el=ui[btn.dataset.target];clampInput(el,Number(el.value)+Number(btn.dataset.delta));}));
function applyPreset(values){Object.entries(values).forEach(([id,v])=>clampInput(ui[id],v));}
document.getElementById('natural').onclick=()=>applyPreset({cadence:176,leg:42,arm:34,elbow:90,freq:1,phaseOff:0,torso:58});
document.getElementById('highKnee').onclick=()=>applyPreset({cadence:180,leg:56,arm:44,elbow:88,freq:1,phaseOff:0,torso:62});
document.getElementById('strongArms').onclick=()=>applyPreset({cadence:176,leg:42,arm:52,elbow:82,freq:1.05,phaseOff:0,torso:62});
document.getElementById('quietArms').onclick=()=>applyPreset({cadence:176,leg:42,arm:14,elbow:95,freq:1,phaseOff:0,torso:45});
refreshLabels();
document.getElementById('front').onclick=()=>{camera.position.set(0,2.7,6.8);controls.target.set(0,1.55,0);controls.update()};
document.getElementById('side').onclick=()=>{camera.position.set(6.7,2.55,.35);controls.target.set(0,1.55,0);controls.update()};
...
function pose(p){const cadence=+ui.cadence.value,legA=+ui.leg.value,armA=+ui.arm.value,elbow=+ui.elbow.value,freq=+ui.freq.value,off=deg(+ui.phaseOff.value),comp=+ui.torso.value/100;const omegaStep=(cadence/60)*Math.PI;const qL=p,qR=p+Math.PI;
...
function animate(){requestAnimationFrame(animate);const dt=Math.min(clock.getDelta(),.04);phase+=dt*((+ui.cadence.value)/60)*Math.PI;pose(phase);const pct=((phase%(2*Math.PI))/(2*Math.PI)*100+100)%100;cycleVal.textContent=`${pct.toFixed(0)}%`;cycleFill.style.width=`${pct}%`;controls.update();renderer.render(scene,camera)} animate();