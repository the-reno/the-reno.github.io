...
async function loadArticle(topic){
 ...
 try{
  ...
  if(controller.signal.aborted||currentTopic?.id!==topic.id||$('#source-content')!==currentHost)return;
  currentHost.innerHTML=`<div class="load-state" role="status"><p>The article could not be loaded. You can retry or read the published page directly.</p><button type="button" id="retry-source">Retry article</button> <a class="source-link" href="${esc(sourceURL(topic))}" target="_blank" rel="noopener noreferrer">Open original page ↗</a></div>`;
...
function renderReader(topic){
 ...
 $('#reader-page').innerHTML=`<div class="reader-top"><a class="quiet-link" href="${esc(lastBrowseHash)}"><svg class="icon" aria-hidden="true"><use href="#i-back"/></svg>Back to exploring</a></div><div class="reader-layout"><aside class="reader-sidebar" aria-label="Article navigation"><p class="eyebrow">In this topic</p><nav class="toc" id="reader-toc" aria-label="On this page"><a href="#topic/${topic.id}/article-start" data-anchor="article-start" class="is-active">Overview</a>${topic.type==='Interactive'?`<a href="#topic/${topic.id}/experiment" data-anchor="experiment">The experiment</a>`:''}</nav><div class="reader-status"><a class="source-link" href="${esc(sourceURL(topic))}" target="_blank" rel="noopener noreferrer">Open original ↗</a></div></aside><article aria-labelledby="reader-title" id="article-start"><header class="reader-head"><div class="eyebrow">${s.name} &nbsp; — &nbsp; ${topic.type}</div><h1 id="reader-title" tabindex="-1">${esc(topic.title)}</h1><p class="reader-deck">${esc(topic.description)}</p><div class="reader-byline"><span>Rafael Renó</span><span>${topic.status||'Published on Ronu'}</span></div></header><div class="article-body">${topic.type==='Article'?'<div class="source-content" id="source-content"></div>':`<section id="experiment" class="article-section"><div class="reader-hero-art">${artSvg(topic.art)}<span class="art-label">Schematic illustration / Not model output</span></div><button type="button" class="launch-button" id="launch-experiment">Open experiment ${arrow}</button><div id="app-mount"></div></section>`}<p class="source-credit"><a href="${esc(sourceURL(topic))}" target="_blank" rel="noopener noreferrer">${topic.type==='Article'?'Original article':'Open experiment separately'} ↗</a></p>${related?`<div class="related"><div class="eyebrow">Keep exploring / ${s.name}</div><a class="related-link" href="#topic/${related.id}"><div><strong>${esc(related.title)}</strong><p>${esc(related.description)}</p></div>${arrow}</a></div>`:''}</div></article></div>`;
 document.title=topic.title+' — Ronu.one';
...
function launchExperiment(topic){
 ...
 $('#app-mount').innerHTML=`<div class="original-app" id="original-app"><div class="app-controls"><span class="reader-progress-text">Interactive</span><div><button type="button" id="expand-app" aria-pressed="false">Expand</button> <button type="button" id="close-app">Close</button> <a class="source-link" href="${esc(sourceURL(topic))}" target="_blank" rel="noopener noreferrer">Open separately ↗</a></div></div><iframe title="${esc(topic.title)} — original interactive" src="${esc(sourceURL(topic))}" referrerpolicy="no-referrer" sandbox="allow-scripts allow-same-origin allow-forms allow-popups allow-popups-to-escape-sandbox allow-downloads" allow="fullscreen"></iframe><p class="app-note">If the model does not display, use “Open separately”. Press Escape to leave expanded view.</p></div>`;
