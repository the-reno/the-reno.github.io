...
def build_workbook():
    ...
    for label, row_num in input_rows.items():
        ws.cell(row_num, 2, label)
        ws.cell(row_num, 2).font = bold_font
        ws.cell(row_num, 2).fill = light_blue
    ...
    # User-entered coupon payment dates on the left side.
    # User enters contractual coupon dates by instrument.
    ws["B15"] = "Coupon Payment Dates"
    ...
    # Holiday calendar moved to the right side of the cash flow table
    ...
    for r, h in enumerate(sample_holidays, start=cf_start_row + 1):
    ...
    holiday_range = f"${holiday_col_letter}${cf_start_row + 1}:${holiday_col_letter}$500"

    while current <= end_date:
        ws.cell(row, cf_start_col, current)

        # Day Type uses the shared holiday calendar on the right side.
        ws.cell(
            row,
            cf_start_col + 1,
        ...
        # - Coupon contractual dates are entered by user in the payment-date table.
        # - Coupon settlement occurs on the first business day on/after the contractual payment date.
        # - Principal settlement occurs on the first business day on/after maturity date.
        for i in range(len(instruments)):
            ...
            formula = (
                ...
                f'IF(SUMPRODUCT(--(WORKDAY({payment_date_range}-1,1,{holiday_range})={row_date}))>0,'
    ...
    ws.column_dimensions[holiday_col_letter].width = 18
    ...
    for r in range(cf_start_row + 1, 501):
