...
def build(path="NII_Engine_v2.xlsx"):
    ...
    put(ws,6,5,'=RBuildCurve("curve1",DATE(2026,6,15),DATE(2030,12,31),3.80,INPUTS!D4:E7,INPUTS!A4:B9)',MONO)

    put(ws,8,1,"2 · ACCRUE",SUB); put(ws,8,3,"Expected",SUB); put(ws,8,4,"Check",SUB); put(ws,8,5,"PASTE THIS",SUB)
    cases=[("Period simple (Jul-Oct)",0.853750,'=RAccrue(DATE(2026,7,1),DATE(2026,10,1),100,"SIMPLE",$B$6)'),
    ("Period compound (Jul-Oct)",0.857325,'=RAccrue(DATE(2026,7,1),DATE(2026,10,1),100,"COMPOUND",$B$6)')]
    r=9
    ...
    for nm,exp,f in [("FIXED leg",1.017187,'=RSwapLeg(DATE(2026,7,1),DATE(2026,8,1),375,3.15,$B$6,"FIXED")'),
    ("FLOAT leg",1.142773,'=RSwapLeg(DATE(2026,7,1),DATE(2026,8,1),375,3.15,$B$6,"FLOAT")'),
    ("NET",-0.125585,'=RSwapLeg(DATE(2026,7,1),DATE(2026,8,1),375,3.15,$B$6,"NET")')]:
        put(ws,r,1,nm); x=put(ws,r,2,None,fill=YEL,box=True); x.number_format="0.000000"
        put(ws,r,3,exp,nf="0.000000"); put(ws,r,4,'=IF(ABS(B%d-C%d)<0.000001,"OK","check")'%(r,r),SUB)
        put(ws,r,5,f,MONO); r+=1

    put(ws,r+1,1,"4 · INSPECT — GET spills provenance + the daily strip",SUB); r+=2
    put(ws,r,1,"Curve table"); put(ws,r,5,"=RBuildCurve("info",$B$6,$B$6,0,$B$6,$B$6)",MONO); r+=1
